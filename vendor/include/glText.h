...
#define _gltFontGlyphLength (_gltFontGlyphMaxChar - _gltFontGlyphMinChar + 1)
	...
	GLT_API GLboolean gltSetText(GLTtext* text, const char* string)
	{
		...
		if (string)
			strLength = (GLsizei)strlen(string);
		...
		if (strLength)
		{
			...
			text->_text = (char*)malloc((long long)(strLength + 1) * sizeof(char));

			if (text->_text)
			{
				memcpy(text->_text, string, (long long)(strLength + 1) * sizeof(char));
...
#define _gltDrawText() \
	...
	GLT_API void _gltUpdateBuffers(GLTtext* text)
	{
		...
		GLfloat* vertices = (GLfloat*)malloc((long long)vertexCount * vertexSize * sizeof(GLfloat));
		...
		text->vertexCount = (long long)vertexCount;
		text->_vertices = vertices;

		glBindBuffer(GL_ARRAY_BUFFER, text->_vbo);
		glBufferData(GL_ARRAY_BUFFER, (long long)vertexCount * _GLT_TEXT2D_VERTEX_SIZE * sizeof(GLfloat), vertices, GL_DYNAMIC_DRAW);
...
#define _gltFontGlyphDataCount 387
	...
	GLT_API GLboolean _gltCreateText2DFontTexture(void)
	{
		...
		GLubyte* texData = (GLubyte*)malloc((long long)texAreaSize * texPixelComponents * sizeof(GLubyte));
